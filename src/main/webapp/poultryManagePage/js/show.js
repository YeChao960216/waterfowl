...
$(function () {
    function showInfo(pageNum){//显示数据
        $.getJSON("/waterfowl/poultry/show", {pageNum: pageNum}, function (json) {
            function divPage(){
            ...
            function thisPage(){//展示页面
                var innerlist="<caption>家禽入厂记录一览</caption><tr><th>入厂记录编号</th><th>记录日期</th>" +
                    "<th>家禽种类</th><th>家禽数量</th><th>计数单位</th><th>来源厂商</th><th>厂商电话</th>" +
                    "<th>记录者编号</th><th>负责人编号</th><th>备注</th><th>操作</th>";
                for(var key in json.list) {
                    innerlist += "<tr>";
                    innerlist += "<td>" + json.list[key].idPoultry + "</td>";
                    innerlist += "<td>" + exTimeStamp(json.list[key].recordDate) + "</td>";
                    innerlist += "<td>" + json.list[key].type + "</td>";
                    innerlist += "<td>" + json.list[key].quantity + "</td>";
                    innerlist += "<td>" + json.list[key].unit + "</td>";
                    innerlist += "<td>" + json.list[key].associatedFirm + "</td>";
                    innerlist += "<td>" + json.list[key].phone + "</td>";
                ...
                function Del(ideleteId){//删除给定记录
                    $.getJSON("/waterfowl/poultry/delete/"+ideleteId,function (json) {
                ...
                function ackEdi(){//edit相应操作
                    ...
                    if(confirm("前往修改记录"+ideditId)){
                        sessionStorage.idedit = ideditId;
